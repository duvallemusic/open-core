# OpenCore — Roadmap Revisado v2.1
...
**Documentos relacionados:** Manifesto v1.1 · Arquitetura v1.0.2 · ADR-015 v1.1  
**Histórico:** supersede Roadmap v2.0 (`05_Roadmap_OpenCore_v2.0.md`), que incorporava parcialmente o ADR-015 v1.
...
- Module Host (supervisão de módulos em processo — hipótese ADR-015);
- banco e migrações;
- barramento de eventos;
- logs e diagnóstico;
- contratos mínimos de segurança e integridade.

**Nota:** isolamento por processo = isolamento de falhas, não sandbox completa de SO (ADR-015 v1.1).
...
- **Spike 10 (ADR-015 v1.1):** módulo em processo **headless** em **Python** (preferência), via OpenCore Module Protocol (stdio + framing + JSON-RPC ou subconjunto); storage mediado sem SQL genérico; comparação Opção A vs B; sem `ui_schema`; sem segundo SDK.
...
- **caso o ADR-015 seja aceito, ao menos um módulo de domínio real da distribuição deverá utilizar `execution.mode: process`**;
...
- módulo simples **em processo** na primeira linguagem externa aceita (preferência: Python), se ADR-015 estiver aceito;
- observabilidade e CI;
- revisão de código assistida.

Se o Spike 10 / ADR-015 não tiver sido aceito, a trilha de processo permanece fora da coorte e o piloto documenta o adiamento.
...
2. **Projeto final ou capstone:** construção de um módulo de domínio completo — preferencialmente via SDK de processo quando ADR-015 estiver aceito, para maximizar acessibilidade educacional sem reduzir padrões de teste, revisão e documentação.
...
### Mudanças específicas da v2.1 (ADR-015 v1.1)

9. **Spike 10 na Etapa 1** com critério de saída explícito (aceito / rejeitado / adiado).
10. **Portaria (Etapa 2)** exige ≥ 1 módulo de domínio real em processo se ADR-015 for aceito.
...
2. Arquitetura v1.0.2 + ADR-015 v1.1;
3. Comunidade e governança;
4. Plano institucional;
5. Roadmap v2.1 consolidado.
