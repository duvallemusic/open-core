...
9. obter uma instalação funcional das distribuições oficiais sem precisar montar manualmente dependências técnicas;
10. conhecer previamente quais módulos, permissões, transmissões externas e requisitos compõem a instalação;
11. reconstruir ou reproduzir a configuração instalada a partir de manifesto e lockfile documentados.
...
- instalar o sistema sem configurar manualmente o OpenCore Runtime, banco de dados, containers ou dependências;
...
Usuários não devem ser obrigados a conhecer nomes de módulos para encontrar uma distribuição adequada. Necessidades do negócio devem ser traduzidas para capacidades técnicas.

Recomendações podem utilizar automação ou IA, mas a composição final deve ser validada por regras determinísticas. A recomendação não pode instalar silenciosamente componentes incompatíveis, não verificados ou não solicitados.
...
- uma experiência em que o usuário precise compreender a arquitetura para instalar uma distribuição;
- IA como substituta da validação de compatibilidade;
- tratar um catálogo de módulos como garantia de que qualquer combinação funciona;
- geração pública arbitrária de builds antes de existir matriz de compatibilidade, assinatura e controle de custo;
- esconder componentes comerciais ou transmissões externas dentro de recomendações automáticas.
