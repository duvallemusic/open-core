# Arquitetura OpenCore — Versão 1.1
...
**Observação:** decisões tecnológicas marcadas como provisórias somente serão confirmadas após os spikes técnicos. Esta revisão incorpora a matriz de classificação arquitetural, o contrato de portabilidade e exclusão por módulo, níveis de confiança, testes arquiteturais e time-box dos spikes.
...
O formato acima é ilustrativo. YAML, TOML ou outro formato somente será confirmado após validação técnica.
...
Na primeira versão, os módulos deverão preferencialmente ser:

- componentes Rust conhecidos no momento da compilação; ou
- componentes registrados estaticamente em uma distribuição.

A primeira versão não deverá assumir:

- ABI binária estável;
- bibliotecas dinâmicas arbitrárias;
- download e execução automática de módulos externos;
- compatibilidade ilimitada entre versões;
- execução segura de código não confiável.

A existência de um contrato de módulo não implica, por si só, um sistema público de plugins dinâmicos.
...
| ADR-005 | Registro estático de módulos antes de plugins dinâmicos | Proposto |
...
**Limite total inicial:** 20 pessoa-dias. Para trabalho individual, a referência é quatro semanas úteis. Um time pequeno poderá executar partes em paralelo, mas deverá manter a fatia vertical compartilhada.
...
## 31. Critérios de saída da Arquitetura v1.1
...
**Mitigação:** iniciar com registro estático e extrair o SDK após a primeira fatia vertical.
...
**Mitigação:** medir onboarding, documentação e produtividade durante os spikes; comparar alternativas caso os resultados sejam inadequados.
...
1. aceitar, alterar ou rejeitar formalmente a matriz de classificação e o contrato de portabilidade desta v1.1;
2. criar os ADRs iniciais, com prioridade para ADR-015 a ADR-020;
3. extrair três especificações normativas: `module-manifest-v0`, `data-portability-v0` e `module-trust-v0`;
4. criar a matriz executável de dependências permitidas e proibidas;
5. definir a convenção inicial de migrações, backup, restauração e exclusão;
6. preparar a estrutura mínima do monorepo e os templates Apache 2.0 da trilha educacional;
7. converter os nove spikes em backlog time-boxed, preservando o limite de 20 pessoa-dias;
8. iniciar a fatia vertical experimental e realizar o primeiro checkpoint de onboarding logo após o Spike 01;
9. executar os demais spikes sobre a mesma fatia vertical;
10. registrar resultados mensuráveis e revisar os ADRs condicionados;
11. revisar esta arquitetura para a versão 1.2 somente quando as evidências exigirem alteração;
12. iniciar a fatia vertical definitiva do OpenCore Portaria apenas após a decisão documentada sobre a stack.

---

## 36. Declaração arquitetural

O OpenCore será desenvolvido inicialmente como um monólito modular, local e multiplataforma, composto por um runtime mínimo, módulos-base, módulos de domínio e distribuições testadas.
...
A primeira implementação deverá priorizar componentes T3 registrados estaticamente, SQLite, comunicação local, adaptadores opcionais e empacotamento nativo. Plugins dinâmicos, marketplace, microserviços e infraestrutura obrigatória de nuvem permanecerão fora do escopo até que necessidades reais justifiquem sua adoção.
