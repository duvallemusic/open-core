# Arquitetura OpenCore — Versão 1.0.2

**Status:** histórico — supersedida pela Versão 1.2 (linha divergente; não usar)  
**Base estratégica:** Manifesto OpenCore v1.1  
**Roadmap relacionado:** OpenCore — Roadmap Revisado v2.1  
**Finalidade:** definir as fronteiras arquiteturais iniciais do OpenCore e preparar a validação técnica da Etapa 1  
**Observação:** decisões tecnológicas marcadas como provisórias somente serão confirmadas após os spikes técnicos.  
**ADR recente:** ADR-015 v1.1 — módulos nativos, módulos em processo e protocolo neutro de linguagem.  
**Histórico imediato:** supersede a Versão 1.0.1 (`02_Arquitetura_OpenCore_v1.0.1.md`).
...
O formato acima é ilustrativo. YAML, TOML ou outro formato somente será confirmado após validação técnica. A classe `process` e o protocolo neutro de linguagem estão condicionados ao ADR-015 v1.1 e ao Spike 10.
...
A primeira versão distinguirá duas classes técnicas de módulo, conforme ADR-015 v1.1:
...
- módulos em processo, mesmo oficiais, operarão com menor confiança implícita: permissões explícitas no protocolo, isolamento de falhas e mediação de storage;
- **não** se deve documentar nem apresentar o isolamento por processo como sandbox capaz de conter código malicioso ou não confiável.

Essa disciplina permitirá evolução futura para níveis diferentes de confiança (oficial, verificado, independente) e, se necessário, sandbox de SO em ADR futura.
...
Um spike posterior poderá validar UI declarativa (tabelas, formulários, rotas) renderizada por componentes oficiais, evitando transformar o protocolo em construtor low-code prematuro. Injeção arbitrária de código Slint a partir de linguagens externas permanecerá fora do escopo até ADR específico.
...
| ADR-015 | Módulos nativos Rust, módulos em processo e protocolo neutro (v1.1: isolamento ≠ sandbox; execution; storage tipado) | Proposto, condicionado a spike |
...
### 28.2.1 Módulos em processo (ADR-015 v1.1)
...
### Spike 10 — Módulo em processo (ADR-015 v1.1)
...
**Mitigação:** Spike 10 medir tamanho; PATH só para desenvolvimento; distribuições oficiais exigem interpretador empacotado ou executável autônomo; não aceitar ADR-015 sem plano de empacotamento de produto.
...
**Mitigação:** declaração explícita no ADR-015 v1.1 e nesta arquitetura; módulos em processo apenas oficiais/verificados/confiáveis na v0; sandbox de SO em ADR futura se houver necessidade de código não confiável.
...
1. criar os ADRs iniciais, incluindo o ADR-015 v1.1;
...
6. preparar a estrutura do monorepo (`native-rust`, `protocol`, `process-*`, `conformance-tests`);
7. criar o backlog detalhado dos spikes, incluindo Spike 10 (headless) e Spike 11 (UI declarativa);
...
A primeira implementação deverá priorizar módulos nativos registrados de forma controlada, SQLite mediado pelo runtime, comunicação local e empacotamento nativo. Módulos em processo, comunicando-se pelo OpenCore Module Protocol, constituem hipótese oficial condicionada ao ADR-015 v1.1 e ao Spike 10 — voltada a domínio, integrações e contribuição educacional, sem substituir Rust no núcleo e **sem** apresentar isolamento de processo como sandbox de segurança.
...
|---|---|---|
| 1.0 | Histórico (conteúdo incorporado em 1.0.1) | Primeira consolidação das fronteiras arquiteturais. |
| 1.0.1 | Histórico | Primeira incorporação do ADR-015 v1. Arquivo: `02_Arquitetura_OpenCore_v1.0.1.md`. |
| 1.0.2 | Proposta para revisão | ADR-015 v1.1: isolamento ≠ sandbox; `execution` canônico; storage tipado A/B; SDK nativo ≠ processo; empacotamento oficial; UI fora do Spike 10; ciclo operacional no backlog. |
